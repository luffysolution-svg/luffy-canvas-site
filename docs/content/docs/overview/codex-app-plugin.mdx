---
title: Codex App 插件
description: 在 Codex App 中安装并使用 luffy-canvas-site 插件
---
...
```text
请从 https://github.com/luffysolution-svg/luffy-canvas-site.git 安装 luffy-canvas-site Codex 插件。
请 clone 仓库到 ~/plugins/luffy-canvas-site，确认 plugins/infinite-canvas/.codex-plugin/plugin.json 存在，
...
```bash
mkdir -p ~/plugins
git clone https://github.com/luffysolution-svg/luffy-canvas-site.git ~/plugins/luffy-canvas-site
```
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "local",
        "path": "./plugins/luffy-canvas-site/plugins/infinite-canvas"
      },
      "policy": {
...
插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开 `https://luffy-canvas-site.netlify.app/` 并自动新建、连接画布。只有明确要求使用本地项目时，插件才会启动本地前端。
