...
function createPluginHttp(config: AiConfig, options?: RequestOptions): PluginHttp {
    const run = async (method: "get" | "post", path: string, body: unknown, opts?: PluginHttpOptions) => {
        ...
        const response = await axios.request({
            ...
            params: opts?.params,
            headers: pluginHeaders({ ...(config.authType === "none" ? {} : { Authorization: `Bearer ${config.apiKey}` }), ...opts?.headers }, method === "post" && !isForm && body !== undefined),
 ...
 *   prompt / images / mask / videos / audios / messages / params —— 本次请求的输入
 *   model / baseUrl / apiKey / authType / systemPrompt —— 当前渠道信息
...
export const PLUGIN_VARIABLES: PluginVariable[] = [
    ...
    { name: "mask", type: "string", desc: "蒙版图 dataURL（蒙版编辑时有值）", capabilities: ["image"] },
    ...
    { name: "authType", type: '"bearer" | "none"', desc: "渠道鉴权方式；none 时不要发送 Authorization" },
    { name: "systemPrompt", type: "string", desc: "系统提示词原文" },
    { name: "http", type: "object", desc: "便捷请求：http.post(path, body, {headers,params,responseType})、http.get(path, opts)、http.url(path)；bearer 渠道默认带 Authorization，可用 headers 覆盖；none 渠道不带鉴权头；path 相对时按 baseUrl 拼 /v1" },
...
if (mask) form.set("mask", await (await fetch(mask)).blob(), "mask.png");
...
const data = await request({
  ...
  headers: { "Content-Type": "application/json", ...(authType === "none" ? {} : { "x-goog-api-key": apiKey }) },
...
return (data.candidates || [])
  ...
  .map((img) => \`data:\${img.mimeType || img.mime_type || "image/png"};base64,\${img.data}\`);`,
    ...
    video: [
        {
            label: "OpenAI 规范",
            script: `// 视频（脚本内部自行轮询）。可用：prompt、images/videos/audios、params{seconds,size,resolution,ratio}
const headers = { "Content-Type": "application/json", ...(authType === "none" ? {} : { Authorization: \`Bearer \${apiKey}\` }) };
...
const headers = { "Content-Type": "application/json", ...(authType === "none" ? {} : { "x-goog-api-key": apiKey }) };
const instance = { prompt };
const first = images[0] && images[0].match(/^data:([^;]+);base64,(.*)$/);
if (first) instance.image = { inlineData: { mimeType: first[1], data: first[2] } };
...
return { blob: await request({ method: "get", url: uri, headers: authType === "none" ? {} : { "x-goog-api-key": apiKey }, responseType: "blob" }) };`,
        },
    ],
    audio: [
        {
