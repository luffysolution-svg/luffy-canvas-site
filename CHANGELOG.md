...
+ [新增] 补全 Netlify 并行部署与同源图片代理 Function，保留 SPA 路由、现有 Vercel 部署及浏览器直连 AI 工作流。
+ [新增] AI 创作审核台 Phase 2 新增五平台精确画板、1–10 页智能拆页、统一风格批量生成与单页重绘、多平台 PNG/ZIP 导出，以及带页级溯源的资产和 Canvas 批量插入。
+ [新增] 新增 AI 创作审核台 Phase 1，支持结构化方案与提示词双重人工审核、2–4 个候选对比、问题反馈、历史恢复及带完整溯源的画布插入。
+ [新增] 生图渠道新增 OpenRouter Chat Completions 与火山方舟 / Seedream 预设，并为创作候选统一补充模型、参数和提示词版本元数据。
...
+ [调整] `@luffysolution/canvas-agent@0.2.0` 已公开发布，网页、插件和集成文档统一改用固定 npm 版本。

## v0.10.0 - 2026-07-26

+ [修复] 远程图片直连受 CORS 拦截时改由受限的同源流式代理下载，使下载、保存到本地、添加到资产和加入参考图可继续使用。
+ [修复] 生图与改图按渠道能力选择远程 URL 或 Base64 返回，远程结果先展示、使用真实链接打开原图，并仅在用户下载、保存或引用时本地化，同时保留链接有效期与渠道任务信息。
+ [优化] 生图请求统一使用批量能力与全局并发队列，加入瞬时错误退避重试、分阶段状态和参考图派生优化，生成成功不再因本地持久化失败而误报失败。
+ [调整] Codex 插件、画布入口及文档中的在线站点地址统一切换为 Vercel 正式域名。
...
+ [调整] 单个模型可声明生图、视频、文本、音频多种能力，并为每种能力分别配置调用脚本；脚本可读取蒙版、参考视频和参考音频。
+ [优化] 视频参数按提供商能力展示，非方形生图结果不再裁切，并加强脚本语法校验、风险提示和已删除渠道保护。
+ [优化] 按模型精确限制 Gemini 图像比例/清晰度、Sora 尺寸/时长、Veo 与 Wan 参考素材及分辨率；Qwen 大文件或越界尺寸会在请求前明确提示。
